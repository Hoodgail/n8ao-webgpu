...
export function neuralKernel(model) {
    ...
    function createState() {
        return {
            ...
            summary: Array.from({ length: 4 }, () => Array.from({ length: 8 }, () => float(0).toVar())),
        };
    }
    function consume(state, token) {
        ...
        for (let q = 0; q < 4; q++) {
            ...
            const oldScale = exp(state.maxima[q].sub(maximum)).toVar(), newScale = exp(score.sub(maximum)).toVar();
            state.summary[q].forEach((v, i) => v.assign(v.mul(oldScale).add(value[i].mul(newScale))));
            state.denominators[q].assign(state.denominators[q].mul(oldScale).add(newScale));
            state.maxima[q].assign(maximum);
        }
    }
    function tap(state, raw) {
        const scaled = raw.map((v, i) => v.mul(m.tapFeatureInverseStandardDeviation[i]));
        consume(state, dense(dense(scaled, m.tapInputWeights, 9, tapBias, s.tapInputWeight, true), m.tapOutputWeights, 8, m.tapOutputBias, s.tapOutputWeight, true));
    }
    function finish(state, baseline, radius, falloff) {
        const raw = [baseline, radius.max(1e-6).log(), falloff.max(1e-6).log()];
        const scaled = raw.map((v, i) => v.mul(m.globalFeatureInverseStandardDeviation[i]));
        consume(state, dense(scaled, m.globalWeights, 3, globalBias, s.globalWeight, true));
        const flat = state.summary.flatMap((row, q) => row.map((v) => v.div(state.denominators[q].max(1e-12))));
