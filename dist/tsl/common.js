import { Fn, abs, cross, dot, float, ivec2, log2, pow, screenCoordinate, vec2, vec3, vec4, } from "three/tsl";
...
export const legacyUV = (uv) => vec2(uv.x, uv.y.oneMinus());
...
export const legacyFragmentPixel = (resolution) => vec2(screenCoordinate.x, resolution.y.sub(screenCoordinate.y));
export const safeNormalize = (v, fallback = vec3(0, 0, 1)) => dot(v, v)
    .greaterThan(1e-12)
    .select(v.mul(dot(v, v).max(1e-12).inverseSqrt()), fallback);
export const rotate2 = (v, c, s) => vec2(c.mul(v.x).add(s.mul(v.y)), s.negate().mul(v.x).add(c.mul(v.y)));
export function createGeometry(U, options) {
    const { depthType, orthographic } = options;
    const background = (d) => d.equal(depthType === DepthType.Reverse ? 0 : 1);
    const linearDepth = (d) => {
        if (depthType !== DepthType.Log || orthographic)
            return d;
    ...
    const view = Fn(([depth, uv]) => {
        ...
        if (!orthographic && depthType !== DepthType.Log) {
            ...
            const p = vec3(matrixColumn(Q, 0).x.mul(clip.x).add(matrixColumn(Q, 3).x), matrixColumn(Q, 1).y.mul(clip.y).add(matrixColumn(Q, 3).y), matrixColumn(Q, 3).z);
            return p.mul(float(1).div(matrixColumn(Q, 2).w.mul(z).add(matrixColumn(Q, 3).w)));
        }
    ...
    const distance = (d, uv) => {
        if (depthType === DepthType.Reverse)
            return view(d, uv).z.negate();
        if (orthographic)
            return U.near.add(d.mul(U.far.sub(U.near)));
    ...
    const load = (tex, pixel, size) => tex.load(ivec2(clampPixel(pixel, ivec2(0), size.sub(1)))).r;
    const normal = (tex, uv, size) => Fn(() => {
        ...
        const actualSize = ivec2(tex.size(float(0))).toVar();
        const p = ivec2(legacyUV(uv).mul(size)).toVar();
        const fetchGL = (x, y) => load(tex, ivec2(p.x.add(x), actualSize.y.sub(1).sub(p.y.add(y))), actualSize);
        const c = fetchGL(0, 0).toVar();
        const l1 = fetchGL(-1, 0).toVar(), l2 = fetchGL(-2, 0).toVar();
        const r1 = fetchGL(1, 0).toVar(), r2 = fetchGL(2, 0).toVar();
        const b1 = fetchGL(0, -1).toVar(), b2 = fetchGL(0, -2).toVar();
        const t1 = fetchGL(0, 1).toVar(), t2 = fetchGL(0, 2).toVar();
        const ce = view(c, uv).toVar(), dx = vec2(float(1).div(size.x), 0), dy = vec2(0, float(1).div(size.y));
        const dl = abs(l1.mul(2).sub(l2).sub(c)), dr = abs(r1.mul(2).sub(r2).sub(c));
        const db = abs(b1.mul(2).sub(b2).sub(c)), dt = abs(t1.mul(2).sub(t2).sub(c));
        const dpdx = dl
            .lessThan(dr)
    ...
    const radiusAt = (d, uv, resolution, radius) => U.screenSpaceRadius.select(view(d, uv).distance(view(d, uv.add(vec2(radius, 0).div(resolution)))), radius);
    const falloffAt = (radius) => radius.mul(U.distanceFalloff).mul(U.screenSpaceRadius.select(1, 0.2));
    return {
        background,
        linearDepth,
        view,
