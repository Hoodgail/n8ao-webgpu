...
const booleans = new Set(Object.keys(DEFAULT_CONFIGURATION).filter((k) => typeof DEFAULT_CONFIGURATION[k] === "boolean"));
...
export function validateOption(key, value) {
    ...
    if (booleans.has(key)) {
        ...
        if (!value ||
            !["r", "g", "b"].every((c) => Number.isFinite(value[c])))
            throw new TypeError("color must have finite r, g, and b components (for example, THREE.Color).");
    }
        ...
        throw new RangeError(`${key} must be ${positive.has(key) ? "positive" : "nonnegative"}.`);
    }
}
/** Validate a batch before committing any changes; failed updates are atomic. */
export function mergeConfiguration(previous, patch) {
    ...
    if (patch.neuralDenoise === true && !previous.neuralDenoise) {
        ...
        if (![4, 8, 16].includes(next.denoiseSamples))
            next.denoiseSamples = 8;
    }
    return next;
}
export function neuralIncompatibilities(c) {
    if (!c.neuralDenoise)
        return [];
    return [
...
export class Configuration {
    ...
    constructor(initial = {}, onChange = () => { }) {
        ...
        this.data = mergeConfiguration({ ...DEFAULT_CONFIGURATION, color: { ...DEFAULT_CONFIGURATION.color } }, initial);
    ...
    update(patch) {
        ...
        const changed = Object.keys(next).filter((key) => next[key] !== this.data[key]);
        if (changed.length === 0)
            return;
...
export function createConfiguration(initial = {}, onChange) {
...
export function dimensions(width, height, halfRes = false) {
