import { Fog, FogExp2, Mesh, OrthographicCamera, PerspectiveCamera, } from "three";
import { NodeUpdateType } from "three/tsl";
import { DataTexture, NearestFilter, NoColorSpace, RGBAFormat, RenderTarget, RendererUtils, RepeatWrapping, TempNode, UnsignedByteType, Vector2, Vector4, WebGPUCoordinateSystem, WebGPURenderer, } from "three/webgpu";
import { blueNoise } from "./assets/data.js";
import { Configuration, DISPLAY_MODES, DepthType, STRUCTURAL_KEYS, dimensions, neuralIncompatibilities, qualityPreset, } from "./core/configuration.js";
...
import { createTextures } from "./rendering/Textures.js";
import { outputTexture, sharedContext, } from "./rendering/ThreeCompatibility.js";
import { TransparencyPass } from "./rendering/TransparencyPass.js";
import { createUniforms } from "./rendering/Uniforms.js";
...
export class N8AONode extends TempNode {
    ...
    _external;
    shaders;
    transparency = new TransparencyPass();
    _textureNode;
    _aoTextureNode;
    ...
    constructor(scene, camera, options = {}) {
        ...
        if (!scene?.isScene ||
            ...
            throw new TypeError("N8AO requires a Scene and a perspective or orthographic camera.");
        const { width = 512, height = 512, autoSize = true, requireWebGPU = true, autoDetectTransparency = true, denoiser = null, ...configuration } = options;
        this.scene = scene;
        this.camera = camera;
        ...
        this.settings = new Configuration(configuration, (keys) => {
            this.resetHistory();
            if (keys.some((k) => STRUCTURAL_KEYS.has(k)))
                this._dirty = true;
            if (keys.includes("halfRes"))
                this.setSize(this.width, this.height);
            if (keys.includes("stencil"))
                this._replaceBeauty();
        ...
        this.bluenoise = new DataTexture(blueNoise, 128, 128, RGBAFormat, UnsignedByteType);
        ...
        this._aoTextureNode = outputTexture(this, this.resources.history[0].texture);
    ...
    _assertAlive() {
        if (this._disposed)
            throw new Error("N8AO has been disposed.");
    }
    getTextureNode() {
    ...
    setDisplayMode(name) {
        ...
        if (mode < 0)
            throw new RangeError(`Unknown N8AO display mode: ${name}`);
        this.configuration.renderMode = mode;
        return this;
    }
    setQualityMode(name) {
    ...
    setInputTextures(colorTexture, depthTexture) {
        this._assertAlive();
        if (!colorTexture?.isTexture || !depthTexture?.isTexture)
            throw new TypeError("setInputTextures requires color and depth textures.");
    ...
    _replaceBeauty() {
        if (!this.resources.beauty)
            return;
        this.resources.replaceBeauty(this.width, this.height, this.configuration.stencil);
        this._dirty = true;
    }
    detectTransparency() {
    ...
    _updateUniforms(renderer) {
        const c = this.configuration, U = this.U, camera = this.camera;
        ...
        if (renderer.coordinateSystem === WebGPUCoordinateSystem &&
            ...
            U.projection.value.fromArray(gpuProjectionToLegacy(camera.projectionMatrix.elements));
    ...
    updateBefore({ renderer, }) {
        ...
        if (this.requireWebGPU &&
            ...
            throw new Error("Native WebGPU is required; await renderer.init() and check adapter availability.");
        ...
        try {
            ...
            if (this.configuration.autoRenderBeauty) {
                ...
                if (!this._external)
                    throw new Error("autoRenderBeauty=false requires setInputTextures(color, depth).");
            ...
            if (this._dirty) {
                this.shaders.rebuild(this.configuration, this.camera, this.U, this.T, this._sharedContext);
                this._dirty = false;
            }
            renderer.setRenderObjectFunction(null);
            renderer.setClearColor(0, 0);
            if (this.configuration.transparencyAware)
                this.transparency.render(renderer, this.scene, this.camera, this.resources, this.shaders);
            const history = this._history.begin(this.camera.matrixWorldInverse.elements, this.camera.projectionMatrix.elements, this.configuration.accumulate, this.configuration.aoSamples);
            this.frame = history.frame;
            this.U.frame.value = this.frame;
            const incompatibilities = neuralIncompatibilities(this.configuration), neuralKey = incompatibilities.join("; ");
            ...
            if (history.render) {
                ...
                if (this.configuration.halfRes) {
                    this.shaders.draw(renderer, this.shaders.downDepth, this.resources.halfDepth);
                    this.shaders.draw(renderer, this.shaders.downNormal, this.resources.halfNormal);
                }
                ...
                for (let i = 0; i < this.configuration.denoiseIterations; i++) {
                    ...
                    this.shaders.draw(renderer, i === 1 && this.shaders.neural && !incompatibilities.length
                        ? this.shaders.neural
                        : this.shaders.denoise, out);
                    current = out;
                }
                this.T.input.value = current.texture;
                ...
                this.shaders.draw(renderer, this.shaders.accumulate, this.resources.history[next]);
                this._historyIndex = next;
            }
            this.T.history.value = this.aoTexture;
            this._aoTextureNode.value = this.aoTexture;
            this.shaders.draw(renderer, this.shaders.composite, this.resources.output);
            if (this.debugMode) {
