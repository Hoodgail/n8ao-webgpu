...
The package name is `n8ao-webgpu` and access is public. Publishing requires permission to the `@hoodgail` scope.

For a first release, authenticate with npm and publish the verified tarball:

```sh
npm login
npm publish ./hoodgail-n8ao-webgpu-1.0.0.tgz --access public
```
...
Before creating a release, update the version and lockfile, describe changes in CHANGELOG.md, and verify the browser suite. Use tags such as `v1.0.0`. Publishing a version to npm is permanent; do not publish an unverified build.
