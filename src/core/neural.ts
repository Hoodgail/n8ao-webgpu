...
const finiteArray = (a: unknown, n: number): a is number[] =>
  Array.isArray(a) && a.length === n && a.every(Number.isFinite);
export function validateModel(m: NeuralModelData): NeuralModelData {
  if (
    !m ||
...
export function prepareModel(input: NeuralModelData) {
  const m = validateModel(input),
    scales = m.quantization.scales;
  return {
    model: m,
    scales,
    tapBias: foldedBias(
      ...
      m.tapInputBias,
      m.tapFeatureMean,
      m.tapFeatureInverseStandardDeviation,
      8,
      9,
      scales.tapInputWeight,
      { 8: 1 },
    ),
    globalBias: foldedBias(
      m.globalWeights,
      m.globalBias,
      m.globalFeatureMean,
...
function dense(
  ...
  return bias.map((b, row) => {
    let sum = 0;
    for (let j = 0; j < x.length; j++) sum += w[row * width + j] * x[j];
    const y = scale * sum + b;
    return relu ? Math.max(0, y) : y;
  });
}
/** CPU evaluation using numerically stable online softmax. */
export function neuralResidual(
  input: NeuralModelData,
  taps: number[][],
  baselineAO: number,
  worldRadius: number,
  ...
  const tokens = taps.map((tap) => {
    if (!finiteArray(tap, 8))
      throw new TypeError(
        "Each neural tap must contain eight finite features.",
      );
    const x = tap.map((v, i) => v * m.tapFeatureInverseStandardDeviation[i]);
    return dense(
      dense(x, m.tapInputWeights, 9, tapBias, s.tapInputWeight, true),
      m.tapOutputWeights,
      8,
      m.tapOutputBias,
      s.tapOutputWeight,
  ...
  tokens.push(
    dense(
      globalRaw.map((x, i) => x * m.globalFeatureInverseStandardDeviation[i]),
      m.globalWeights,
      3,
      globalBias,
      s.globalWeight,
  ...
  const maxima = new Array(4).fill(-1e30),
    denom = new Array(4).fill(0),
    summary = Array.from({ length: 4 }, () => new Array(8).fill(0));
  for (const token of tokens) {
    ...
    for (let q = 0; q < 4; q++) {
      const score =
        key.reduce((sum, k, i) => sum + k * m.summaryQueries[q * 8 + i], 0) *
        0.3535533905932738;
      const maximum = Math.max(maxima[q], score),
        oldScale = Math.exp(maxima[q] - maximum),
        newScale = Math.exp(score - maximum);
      for (let i = 0; i < 8; i++)
  ...
  const flat = summary.flatMap((row, q) =>
    row.map((x) => x / Math.max(denom[q], 1e-12)),
  );
