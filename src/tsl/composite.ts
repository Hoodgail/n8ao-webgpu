...
export function compositeFragment(
  ...
  return Fn(() => {
    const coord = uv(),
      scene = T.beauty.sample(coord).toVar(),
      depth = T.fullDepth.sample(coord).r.toVar();
    ...
    if (options.halfRes && options.depthAwareUpsampling) {
      If(G.background(depth), () => {
        ...
        const p = G.view(depth, coord).toVar(),
          normal = G.normal(T.fullDepth, coord, U.fullResolution).toVar();
        const radius = G.radiusAt(
          depth,
          coord,
          U.fullResolution,
          U.radius,
        ...
        for (let x = -1; x <= 1; x++)
          for (let y = -1; y <= 1; y++) {
            ...
            const pGL = ivec2(
              legacyUV(coord).mul(U.fullResolution).mul(0.5).add(vec2(x, y)),
            ).toVar();
            const pTop = ivec2(
              pGL.x,
              ivec2(U.targetResolution).y.sub(1).sub(pGL.y),
            ).toVar();
            const pixel = ivec2(
              clampPixel(pTop, ivec2(0), ivec2(U.targetResolution).sub(1)),
            );
            const sampleDepth = T.depth.load(pixel).r.toVar(),
              sample = T.history.load(pixel).toVar();
            const sampleUV = legacyUV(vec2(pGL).div(U.fullResolution.mul(0.5)));
            const sampleNormal = sample.gba.mul(2).sub(1);
            const planeDistance = abs(
              dot(G.view(sampleDepth, sampleUV).sub(p), normal),
            );
            const weight = exp(
              planeDistance.negate().mul(float(1).div(falloff)),
            ).mul(dot(normal, sampleNormal).max(0));
    ...
    If(U.fog, () => {
      ...
      const f = U.fogExp.select(
        float(1).sub(
          exp(
            U.fogDensity.mul(U.fogDensity).mul(distance.mul(distance)).negate(),
          ),
        ),
        smoothstep(U.fogNear, U.fogFar, distance),
      );
    ...
    If(U.renderMode.greaterThanEqual(3), () => {
      If(coord.x.lessThan(0.5), () => {
        ...
        .ElseIf(
          abs(coord.x.sub(0.5)).lessThan(float(1).div(U.fullResolution.x)),
          () => {
    ...
    If(U.gammaCorrection, () => {
      ...
      const low = rgb.mul(12.92),
        high = pow(rgb, vec3(0.41666)).mul(1.055).sub(0.055);
      output.rgb.assign(
        vec3(
          rgb.r.lessThanEqual(0.0031308).select(low.r, high.r),
          rgb.g.lessThanEqual(0.0031308).select(low.g, high.g),
          rgb.b.lessThanEqual(0.0031308).select(low.b, high.b),
        ),
      );
    });
    return output;
