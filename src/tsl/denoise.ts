...
export function denoiseFragment(
  U: Uniforms,
  T: Textures,
  G: GeometryNodes,
  options: ShaderOptions,
  model: NeuralModelData | null = null,
): Node<"vec4"> {
  const neural = model ? neuralKernel(model) : null;
  return Fn(() => {
    const coord = uv(),
      data = T.input.sample(coord).toVar(),
      depth = T.depth.sample(coord).r.toVar();
    const result = vec4(data).toVar();
    If(G.background(depth).not(), () => {
      const normal = data.gba.mul(2).sub(1).toVar(),
        p = G.view(depth, coord).toVar();
      const noise = T.noise
      ...
      const angle = channel.mul(Math.PI * 2).toVar(),
        c = cos(angle),
        s = sin(angle);
      const radius = G.radiusAt(depth, coord, U.resolution, U.radius).toVar(),
        falloff = G.falloffAt(radius).toVar();
      const occlusion = float(data.r).toVar(),
        count = float(1).toVar();
      let state!: ReturnType<NonNullable<typeof neural>["createState"]>;
      let frameNormal: Node<"vec3">,
        tangent: Node<"vec3">,
        bitangent: Node<"vec3">;
      if (neural) {
      ...
      Loop(options.denoiseSamples, ({ i }) => {
        ...
        const offset = vec2(rotated.x, rotated.y.negate())
          ...
          .mul(U.denoiseRadius);
        const sampleUV = coord.add(offset).toVar(),
          sample = T.input.sample(sampleUV).toVar();
        const dn = T.depth.sample(sampleUV).r.toVar(),
          nn = sample.gba.mul(2).sub(1).toVar();
        const delta = G.view(dn, sampleUV).sub(p).toVar(),
          planeDistance = abs(dot(delta, normal)).toVar();
        ...
        if (neural)
          If(G.background(dn).not(), () => {
            const localDelta = vec3(
              dot(tangent, delta),
              dot(bitangent, delta),
              dot(frameNormal, delta),
            )
              .div(radius.max(1e-6))
              .toVar();
      ...
      if (neural)
        occlusion.assign(
          clamp(
            occlusion.add(
              neural.finish(state, occlusion, U.radius, U.distanceFalloff),
            ),
            0,
            1,
          ),
