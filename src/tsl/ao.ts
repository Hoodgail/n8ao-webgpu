    ...
    If(G.background(depth).not(), () => {
      ...
      const normal = (
        options.halfRes
          ...
          : G.normal(T.depth, coord, U.resolution)
      ).toVar();
      const noise = T.noise
        .sample(legacyFragmentPixel(U.targetResolution).div(128))
        .toVar();
      const jitter = fract(
        noise.rg.add(vec2(1.618033988749895, 1.324717957244746).mul(U.frame)),
      ).toVar();
      ...
      const angle = jitter.x.mul(3.1415962 * 2).toVar(),
        c = cos(angle),
        s = sin(angle);
      ...
      const bias = min(0.1, falloff.mul(0.1))
        ...
        .add(U.biasOffset)
        .toVar();
      const move = jitter.y.toVar(),
        occluded = float(0).toVar(),
        total = float(0).toVar();
      Loop(options.aoSamples, ({ i }) => {
        ...
        If(valid, () => {
          ...
          const range = smoothstep(
            0,
            1,
            falloff.div(abs(ds.sub(dw)).max(1e-20)),
          );
          const diff = legacyFragmentPixel(U.targetResolution).sub(
            floor(legacyUV(projected.xy).mul(U.resolution)),
          );
          const weight = range
      ...
      result.assign(
        vec4(
          clamp(float(1).sub(occluded.div(total.max(1))), 0, 1),
